...
test = matrix(scan("data/Leesferry-mon-data.txt"), ncol = 13, 
    byrow = T)
...
#source('skew.s')
May = test[, 5]
xeval = seq(min(May) - 0.25 * sd(May), max(May) + 0.25 * sd(May), 
    length = 100)
nevals = length(xeval)
library(sm)
...
#boxplots of the PDF....
par(mfrow = c(1, 1))
zz = boxplot(split(simpdf, col(simpdf)), plot = F, cex = 1)
zz$names = rep("", length(zz$names))
z1 = bxp(zz, ylim = range(simpdf, xdensityorig), xlab = "", ylab = "", 
    cex = 1.25)
z2 = 1:6
n1 = 1:6
z2[1] = z1[1]
