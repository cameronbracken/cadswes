...
pois.HMM.mle <- function(x, m, lambda0, gamma0, ...) {
    ...
    n <- sum(!is.na(x))
    BIC <- 2 * mllk + np * log(n)
    list(lambda = pn$lambda, gamma = pn$gamma, delta = pn$delta, code = pn$code, mllk = mllk, AIC = AIC, BIC = BIC)
}
...
pois.HMM.generate_sample <- function(n, m, lambda, gamma, delta = NULL) {
    ...
    for (i in 2:n) state[i] <- sample(mvect, 1, prob = gamma[state[i - 1], ])
...
pois.HMM.pseudo_residuals <- function(x, m, lambda, gamma, delta = NULL, ...) {
    if (is.null(delta)) 
        delta <- solve(t(diag(m) - gamma + 1), rep(1, m))
    n <- length(x)
    cdists <- pois.HMM.conditionals(x, m, lambda, gamma, delta = delta, xrange = 0:max(x))$cdists
