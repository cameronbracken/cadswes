...
path <- file.path('data','timeseries','climate_predictors')

vars <- c("Geopotential+Height", "Zonal+Wind", "Meridonal+Wind", "SST")
var_short <- c("gph", "znw", "mdw", "sst")
regions <- read.table('data/box_coords.dat',header=T)
...
w <- options()$width
options('width'=2000)
sink(paste('data/raw_predictors_',years[1]+1,'_',years[2],'.txt',sep=''))
