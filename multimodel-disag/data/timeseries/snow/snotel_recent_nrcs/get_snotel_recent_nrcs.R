...
#
url <- "http://www.wcc.nrcs.usda.gov/ftpref/data/snow/snotel/table/water_year"
dl <- T
# to get the most up to date data, simply run the following commands
#
# $ wget -r -l1 --no-parent -A_s.txt http://www.wcc.nrcs.usda.gov/ftpref/data/snow/snotel/table/water_year/wyoming/
# $ wget -r -l1 --no-parent -A_s.txt http://www.wcc.nrcs.usda.gov/ftpref/data/snow/snotel/table/water_year/colorado/
# $ wget -r -l1 --no-parent -A_s.txt http://www.wcc.nrcs.usda.gov/ftpref/data/snow/snotel/table/water_year/utah/
# $ mkdir raw_snotel
# $ cp www.wcc.nrcs.usda.gov/ftpref/data/snow/snotel/table/water_year/wyoming/* raw_snotel
# $ cp www.wcc.nrcs.usda.gov/ftpref/data/snow/snotel/table/water_year/colorado/* raw_snotel
# $ cp www.wcc.nrcs.usda.gov/ftpref/data/snow/snotel/table/water_year/utah/* raw_snotel
...
snotel.recent <- matrix(NA,ncol=4,nrow=length(yrange))
for(i in 4:7)
    snotel.recent[,i-3] <- apply(datamat[,i,],1,mean,na.rm=T)
rownames(snotel.recent) <- yrange
colnames(snotel.recent) <- month.abb[1:4]

snotel.recent.ts <- ts(snotel.recent,start=yrange[1])

write.table(snotel.recent,quote=F,file='snotel_recent.txt')